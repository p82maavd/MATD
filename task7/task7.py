...
if __name__ == '__main__':
    ...
    if len(sys.argv) >= 0:
        ...
        for f in files:
            ...
            nameoutfile="similardocumentstest.txt"
            outfile= open(nameoutfile,"w")

            #Calculate matrix of frequencies
        ...
        for x in dictionary:
            for y in dictionary[x]:
                sumofrootsquare[y[0]]+=y[1]*y[1]

        for x in sumofrootsquare:
            sumofrootsquare[x]=math.sqrt(sumofrootsquare[x])
        
        #Normalizacion
        for x in dictionary:
            for y in dictionary[x]:
                #sumofrootsquare[y[0]]+=y[1]
                y[1]/=sumofrootsquare[y[0]]

        #Check each file with all others files
        sumatorio=0
        bestfile=[0,0,0]
        for w in files:
            sumatorio=0
            ...
            for z in files:
                ...
                if sumatorio > bestfile[2]:
                    bestfile=[w,z,sumatorio]

            print(bestfile)

            outfile.write(str(bestfile)+"\n")
